...
> **[판정 반영]**으로 표기한다.
>
> Phase-1 규율 전부 승계: `KUMIHO_MEMORY_DECISIONS` 옵트인, 신규 의존성 0, 서버 변경 0,
...
async def code_mine_session(
    ...
    *,
    ...
    # 게이트 밖: {"errors": ["code memory is disabled (set KUMIHO_MEMORY_DECISIONS=1)"]}
...
**(c) consolidation 체인 (이중 옵트인, 기본 OFF)** — `KUMIHO_MEMORY_DECISIONS=1` **그리고**
`KUMIHO_MEMORY_DECISIONS_AUTOMINE=1`일 때만, `consolidate_session()`의 `decompose_and_link`
...
env 추가: `KUMIHO_MEMORY_DECISIONS_AUTOMINE` (기본 unset=off). `KUMIHO_MEMORY_DECISIONS` 마스터
게이트는 그대로 — AUTOMINE은 하위 스위치. (레거시 `KUMIHO_MEMORY_CODE*` 이름은 폴백으로 계속 인식.)
...
- **멱등/크래시**: 마커 완결 재실행 = LLM 0콜; message_count 델타 → 재마이닝; 마커-전
  ...
  **force: origin=session 결정만 deprecate되고 enrich된 커밋-유래 결정은 무접촉
  [판정 반영]**
- **게이트/격리**: `KUMIHO_MEMORY_DECISIONS` off → manager 즉시 반환/MCP 미등록; `AUTOMINE`
