...
**[구현 수정, 2026-07-11 어드버서리얼 리뷰 반영]** Python SDK 클라이언트에 `deprecate_item` API가
현재 없어 (MCP 서버 레벨에만 존재) Phase 1의 `--force`는 **deprecate 없이 재처리**한다: 마커-스킵만
우회하고 노드는 get-or-create로 수렴한다 (같은 title+date → 같은 노드에 수렴; 프롬프트 개선으로
title이 바뀐 경우 새 노드가 생기고 구 노드는 잔존). 프롬프트 업그레이드 후 완전 재캡처가 필요하면
**전용 project 삭제 후 재수집**이 지원 경로다 (도그푸드 스크립트가 이 패턴을 사용).
deprecate-then-rewrite는 클라이언트에 `deprecate_item`이 노출된 뒤 Phase 2에서 구현한다.
